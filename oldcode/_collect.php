...
function readPage($url, $params = array(), $ch = null) {
	$q = '';
...
/*
FOR THE NEW COURSE SCHEDULE DATA SITE (INCOMPLETE)..
...
function dumpCourseInfo_new() {
	$doc = readPage("http://courseschedules.njit.edu/index.aspx", array("semester" => "2011s"));
	$elemList = getCourseLinks($doc);
	$res = array();
	if (count($elemList) > 0) {
		foreach ($elemList as $elem) {
			$href = htmlspecialchars_decode($elem->getAttribute("href"));
			$doc2 = readPage("http://courseschedules.njit.edu/" . $href);
			if (!$doc2) {
	...
	for ($i = 0; $i < count($days); $i++) {
		...
		if (strlen($d) != 1 && $d != "TBA") {
			for ($j = 0; $j < strlen($d); $j++) {
				$dayOfWeek = strpos(" MTWRFS", $d[$j]);
				$slots[] = new TimeSlot($dayOfWeek + 1, $r[0], $r[1], $l); 
			}
		} else {
			$slots[] = new TimeSlot(
				$d == "TBA" ? 0 : (strpos(" MTWRFS", $d) + 1),
				$r[0],
				$r[1],
				$l
			);
...
//		echo "&gt; <b>$nr</b>$var - $str<br/>";
	...
	return array(array_values($courses), $count);
}
			...
			foreach( $crs->sections as $sec ) {
				...
				*/
				...
				if (!$ins_sect_stmt->execute()) {
					echo "$crs->subject.$crs->coursenr$crs->coursevar.$sec->section could not be inserted: $conn->error<br/>\n";
					continue;
				}
				...
				if ($sec->slots && count($sec->slots) > 0) {
					foreach ( $sec->slots as $slot ) {
						...
						if (!$ins_slot_stmt->execute()) {
							...
							echo "Failed to execute insert timeslot statement " . $conn->error;
							$failed = true;
							break;
						}
					}
...
try {
	...
	/*if ($row[3] == $last_update && $row[4] == "0") {
		...
		echo "Extracting new data for $row[1]..$lineEnd";
		if (!IS_CLI)
